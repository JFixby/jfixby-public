...
public class PSDRepacker {
	...
	public static void repackPSD(PSDRepackSettings settings,
			PSDRepackingStatus handler) throws IOException {
		...
		scene2d_output = repacking_output.child(package_name.child(
				Scene2DPackage.SCENE2D_PACKAGE_FILE_EXTENSION).toString());
		scene2d_output.makeFolder();
		related_folders.add(scene2d_output);
		scene2d_output = scene2d_output
		...
		PSDFileContent layers_structure = extractLayerStructures(psd_file,
				raster_names, package_name);

		L.d("---[Packing Layers Structure]--------------------------------------------");
		ConversionResult pack_result = packLayers(layers_structure,
				package_name, scene2d_output, raster_names
		...
		raster_produced = saveRaster(psd_file, raster_names, used_raster,
				raster_folder, package_name, save_raster,
		...
		if (!ignore_atlas && raster_produced) {
			...
			Collection<TextureSlicingResult> structures = decomposeRaster(
					layer_to_file_mapping, tiling_folder, max_texture_size,
					margin);
			...
			for (TextureSlicingResult combo : structures) {
				...
				packed_structures.add(Names
						.newAssetID(composition.composition_asset_id_string));
				requred_rasters.addAll(combo.listProducedTiles());
			}

			if (container.content.size() > 0) {
				AssetID sctruct_package_name = package_name.child("psd").child(
						TextureSlicerSpecs.TILE_MAP_FILE_EXTENSION);
				...
				container_file = container_file.child(sctruct_package_name
						.toString());
				...
				producePackageDescriptor(container_file.parent().parent(),
						StandardPackageFormats.RedTriplane.TiledRaster, "1.0",
						packed_structures, requred_rasters,
			...
			AtlasPackingResult atlas_result = packAtlas(atlas_folder,
					tiling_folder, package_name.child("psd").child("raster")
							.toString());
			...
			atlas_output = repacking_output.child(atlas_name).child(
					PackageDescriptor.PACKAGE_CONTENT_FOLDER);
			...
			producePackageDescriptor(atlas_output.parent(),
					StandardPackageFormats.libGDX.Atlas_GWT, "1.0",
					packed_rasters, JUtils.newList(), atlas_name);
	...
	static private PSDFileContent extractLayerStructures(File psd_file,
			Map<PSDLayer, AssetID> raster_names, AssetID package_name)
			throws IOException {
		int k = 0;
		...
		for (int i = 0; i < rasters.size(); i++) {
			...
			AssetID raster_name = Names.newAssetID(package_name
					+ ".psd.raster_" + k);
	...
	static private ConversionResult packLayers(PSDFileContent layers_structure,
			AssetID package_name, File final_output,
		...
		ConversionResult result = PSDtoScene2DConverter.convert(container,
				package_prefix, root, raster_names);
		...
		String root_file_name = package_prefix.child(
				Scene2DPackage.SCENE2D_PACKAGE_FILE_EXTENSION).toString();
		String data = Json.serializeToString(container);
		...
		for (int i = 0; i < container.structures.size(); i++) {
			AssetID element_id = Names
					.newAssetID(container.structures.get(i).structure_name);
			provisions.add(element_id);
		}

		Collection<AssetID> requred_assets = result.listAllRequredAssets();
		producePackageDescriptor(descriptor,
				Scene2DPackage.SCENE2D_PACKAGE_FORMAT, "1.0", provisions,
				requred_assets, root_file_name);

		return result;

	}

	static private AtlasPackingResult packAtlas(File atlas_folder,
			File sprites, String atlas_file_name) throws IOException {
	...
	static private List<TextureSlicingResult> decomposeRaster(
			Map<PSDLayer, File> layer_to_file_mapping, File tiling_folder,
			int max_texture_size, int margin) throws IOException {
		...
		for (int i = 0; i < layer_to_file_mapping.size(); i++) {
			...
			if (diag > max_texture_size) {
				// decompose
				TextureSlicingResult result = decomposeSprite(png_file,
						tiling_folder, margin, max_texture_size);
				results.add(result);
			} else {
				// copy as is
				File file_to_copy = png_file;
				tiling_folder.getFileSystem().copyFileToFolder(file_to_copy,
						tiling_folder);
			}
		}
		return results;
	...
	static private TextureSlicingResult decomposeSprite(File png_file_path,
			File tiling_folder, int margin, int max_texture_size)
			throws IOException {
		...
		String asset_name = png_file_path.getName().substring(0,
				png_file_path.getName().length() - ".png".length());
	...
	static private boolean saveRaster(File package_root_file,
			Map<PSDLayer, AssetID> raster_names,
			Collection<AssetID> used_raster, File output_folder,
			AssetID package_name, boolean save_raster,
			Map<PSDLayer, File> layer_to_file_mapping,
		...
		for (int i = 0; i < raster_names.size(); i++) {
			...
			saveRaster(png_file_name, layer, output_folder, save_raster,
					layer_to_file_mapping, pack_result);
	...
	static private void saveRaster(String png_file_name, PSDLayer layer,
			File output_folder, boolean save_raster,
			Map<PSDLayer, File> layer_to_file_mapping,
			ConversionResult pack_result) throws IOException {
		...
		SceneStructurePackingResult result = pack_result
				.getStrucutreResultByLayer(layer);

		double scale_factor = result.getScaleFactor();

		Image out;
		if (scale_factor != 1) {
			out = java_image.getScaledInstance(
					(int) (java_image.getWidth() * scale_factor),
					(int) (java_image.getHeight() * scale_factor),
		...
		if (save_raster) {
			...
			ImageProcessing.writeJavaFile(out, output_file, "png");
		}
