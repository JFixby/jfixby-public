...
public class RedRectangle extends VertexMaster implements Rectangle {
	...
	private void update() {
		top_left.relative().set(0 - origin_relative.getX(),
				0 - origin_relative.getY());
		top_right.relative().set(1 - origin_relative.getX(),
				0 - origin_relative.getY());
		bottom_right.relative().set(1 - origin_relative.getX(),
				1 - origin_relative.getY());
		bottom_left.relative().set(0 - origin_relative.getX(),
				1 - origin_relative.getY());
	...
	private void spread(RedVertex spread) {
		spread.relative().setX(
				spread.relative().getX() * this.width);
		spread.relative().setY(
				spread.relative().getY() * this.height);
	}
	...
	public void setOriginAbsolute(double origin_x, double origin_y) {
		...
		if (new_method) {
			this.tmp.set(origin_x, origin_y);
			this.toRelative(tmp);
			this.origin_relative.set(
	...
	public void setOriginRelative(double ORIGIN_POSITION_HORIZONTAL,
		...
		final boolean new_method = !true;
		if (new_method) {
			this.tmp.set(ORIGIN_POSITION_HORIZONTAL, ORIGIN_POSITION_VERTICAL);
			this.toAbsolute(tmp);
		...
		this.origin_relative.set(ORIGIN_POSITION_HORIZONTAL,
				ORIGIN_POSITION_VERTICAL);
		this.update();
	}
	...
	public void toAbsolute(Float2 temp_point) {
		temp_point.set(temp_point.getX() - this.origin_relative.getX(),
	...
	public boolean containsPoint(double canvas_x, double canvas_y) {
		this.triangle_a.A().relative()
				.set(this.top_left.world());
		this.triangle_a.B().relative()
				.set(this.top_right.world());
		this.triangle_a.C().relative()
				.set(this.bottom_right.world());
